...
<body>
    <div class="container">
        ...
        // Get current categories of the film
        Set<Long> currentCategorieIds = new HashSet<>();
        if (film.getCategorieFilms() != null) {
            for (CategorieFilm cf : film.getCategorieFilms()) {
                if (cf.getCategorie() != null) {
                    currentCategorieIds.add(cf.getCategorie().getId());
                }
            }
        }
        %>
        ...
        <form method="post" action="${pageContext.request.contextPath}/films/<%= film.getId() %>">
            <div class="form-group">
                <label for="nom">Film Name:</label>
                <input type="text" id="nom" name="nom" value="<%= film.getNom() %>" required>
            </div>
                ...
                <select id="typeFilm" name="typeFilm.id" required>
                    ...
                    if (typeFilms != null) {
                        for (TypeFilm type : typeFilms) {
                            boolean selected = film.getTypeFilm() != null && 
                                             film.getTypeFilm().getId().equals(type.getId());
                    %>
                ...
                <div class="checkbox-group">
                    ...
                    if (categories != null) {
                        for (Categorie categorie : categories) {
                            boolean checked = currentCategorieIds.contains(categorie.getId());
                    %>
